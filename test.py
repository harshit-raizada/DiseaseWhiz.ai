...
def get_document_date(file_path: str) -> str:
    """
    Extract the creation date from a PDF file.
    ...
    try:
        with open(file_path, 'rb') as file:
            ...
            if '/CreationDate' in pdf.metadata:
                # PDF creation dates are typically in the format "D:YYYYMMDDHHmmSS"
    ...
    # If extraction fails, return the current date
    return datetime.now().strftime('%Y-%m-%d')

def get_document_author(file_path: str) -> str:
    """
    Extract the author from a PDF file.
    Returns the author name as a string, or 'Unknown' if not found.
    """
    try:
        with open(file_path, 'rb') as file:
            pdf = PdfReader(file)
        ...
        for filename in os.listdir(pdf_folder):
            if filename.endswith('.pdf'):
                ...
                for doc in pdf_documents:
                    ...
                    doc.metadata['date'] = get_document_date(file_path)
                    doc.metadata['author'] = get_document_author(file_path)
                    doc.metadata['document_type'] = get_document_type(file_path)
...
def filter_documents(documents: List[Dict], filters: Dict[str, str]) -> List[Dict]:
    """
    Filter documents based on the provided metadata filters.
    """
    filtered_docs = []
...
def get_answer(query: str, filters: Optional[Dict[str, str]] = None) -> Dict:
    # Apply filters to the documents if provided
    if filters:
        filtered_docs = filter_documents(documents, filters)
        # Create a new retriever with filtered documents
        filtered_vectorstore = FAISS.from_documents(filtered_docs, embedding=embeddings)
        filtered_semantic_retriever = filtered_vectorstore.as_retriever(search_type="similarity", search_kwargs={"k": 5})
        filtered_bm25_retriever = BM25Retriever.from_documents(filtered_docs)
        filtered_hybrid_retriever = EnsembleRetriever(
            retrievers=[filtered_bm25_retriever, filtered_semantic_retriever],
            weights=[0.5, 0.5]
        )
        filtered_retriever = ContextualCompressionRetriever(
            base_compressor=compressor,
            base_retriever=filtered_hybrid_retriever
        )
        # Update the RetrievalQA chain with the filtered retriever
        retrievalQA.retriever = filtered_retriever
...
async def ask(query_request: QueryRequest):
    try:
        result = get_answer(query_request.query, query_request.filters)
